  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden soft-mask" aria-hidden="true">
      ...
      <div className="absolute right-[-180px] top-28 hidden w-[760px] origin-top-right rotate-[-3deg] lg:block xl:right-[-230px] xl:top-32 xl:scale-[1.12] min-[1800px]:right-[-90px] min-[1800px]:scale-[1.24]">
        <div className="rounded-ui border border-white/70 bg-white/70 p-3 shadow-deep backdrop-blur-2xl xl:p-4">
          <div className="flex items-center justify-between border-b border-border pb-3 xl:pb-4">
            ...
            <div className="flex gap-2">
              {["New", "Follow up", "Converted"].map((item) => (
                <span key={item} className="rounded-ui border border-white/70 bg-white/72 px-2 py-1 text-xs font-bold text-ink-600 shadow-crisp backdrop-blur">
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-[1fr_0.9fr] gap-3 pt-3 xl:gap-4 xl:pt-4">
            <div className="grid gap-3">
              ...
              <div className="rounded-ui border border-white/70 bg-white/62 p-3 shadow-crisp backdrop-blur xl:p-4">
                ...
                <div className="grid gap-2 xl:gap-2.5">
                  {rows.map(([room, occupancy, status, due, action]) => (
                    <div key={room} className="grid grid-cols-[0.65fr_0.5fr_0.72fr_0.7fr_0.72fr] items-center gap-2 rounded-ui bg-white/82 px-3 py-2 text-xs font-bold text-ink-700 shadow-crisp backdrop-blur xl:px-4 xl:py-2.5">
            ...
            <div className="grid gap-3">
              <div className="rounded-ui border border-white/70 bg-white/72 p-3 shadow-crisp backdrop-blur xl:p-4">
                ...
                <div className="mt-3 grid gap-2">
                  {[
                    ...
                    <div key={title} className="rounded-ui border border-white/70 bg-white/64 p-3 shadow-crisp xl:p-4">
...
function Metric({
  ...
  return (
    <div className="rounded-ui border border-white/70 bg-white/76 p-3 shadow-crisp backdrop-blur">
...
function Signal({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-ui border border-white/70 bg-white/78 p-3 shadow-deep backdrop-blur-xl">
